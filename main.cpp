...
03/09/2024
I am able to make 13 work on an RPi yet haven't decided on
UI elements on phones versus desktops. I did want UI
dependant on screen sizes, but also down to the window
not functioning on RPi so separate versions needed.
Either choose a single file with all the checks, or
Maintain two separate files.

Phone, Desktop... What else?
...
class Game
{
  ...
  public:
    ...
    {
	#if PLATFORM_DESKTOP
		std::cout << "Using landscape dimensions.";
	#elif PLATFORM_MOBILE
		std::cout << "Using portrait dimensions.";
	#elif PLATFORM_UNKNOWN
		std::cout << "Using unknown dimensions.";
	#endif
    }
...
static SDL_Texture *loadImage(SDL_Renderer *renderer, const char *path)
{
    ...
    if (img == NULL)
    {
	std::cout << "Image unavailable." << std::endl;
        return NULL;
    }
...
int Game::init()
{
    ...
    window = SDL_CreateWindow("Hello SDL2", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 360, 800, 0);
    if (window == NULL)
    {
        return -1;
    }
...
void Game::checkInput()
{
    ...
    {
	// make sure we can quit on desktop
	if(e.type == SDL_QUIT)
		isQuit = true;
...
{
    Game game{};
    std::cout << "Size of CGame " << sizeof(game);
